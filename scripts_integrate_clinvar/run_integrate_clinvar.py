...
argparser = argparse.ArgumentParser(description="Integrate variant data with ClinVar information for all samples")
argparser.add_argument("-c","--chunk",required=True, type=str, help="Chunk to process")
...
def run_integrate_clinvar(gene, person):
    person_analysis_folder = os.path.join(analysis_folder, gene, person)
    
    cmd = ["/home/b/buit/miniconda3/envs/HiWi/bin/python", "/mnt/raidproj/proj/projekte/personalizedmed/PPG/miRNAs/scripts_integrate_clinvar/integrate_clinvar_individual.py",
            "-v", os.path.join(person_analysis_folder, "all_variants.tsv")
            ]
...
if __name__ == "__main__":
    ...
    for ID, _ in gene_dict.items():
        print(f"Integrating clinvar for gene: {ID}")
        if chrom != "chrY":
            for person in FAMILY:
                run_integrate_clinvar(ID, person)
        else:
