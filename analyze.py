...
def make_line_plot_cov(df, gene, output_line_plot_path, output_boxplot_path):
    ...
    Args:
        df (pd.DataFrame): DataFrame with coverage data.
    ...
    g = sns.FacetGrid(gene_df, col="Person", col_wrap=2, height=4, aspect=1.5)
    ...
    g.fig.suptitle(f"Coverage for Gene {gene} by Person", y=1.02)
    ...
    sns.boxplot(data=gene_df, x="Person", y="Coverage", color="skyblue")

    plt.xticks(rotation=45)
    plt.ylabel("Coverage")
    plt.xlabel("Person")
    plt.title(f"Coverage Distribution for Gene {gene}")
...
def make_combine_results(gene_name, list_results_files):
    ...
    for file in list_results_files:
        ...
        df = pd.read_csv(file)  # Ensure tab-separated format
...
def find_mendelian_troublesome_positions(df, family_relationships):
    ...
    for _, row in df.iterrows():
        ...
        for family in family_relationships:
            ...
            p1_alleles = set(father_genotype.split('/'))  # Use set to handle homo/hetero alleles
            p2_alleles = set(mother_genotype.split('/'))
            ...
            allowed_alleles = p1_alleles.union(p2_alleles)

            # Check if the child's alleles are valid (they must be a subset of the allowed alleles)
            if not child_alleles.issubset(allowed_alleles):
...
def run_analysis(gene_name:str, base_output_folder:str):
    ...
    list_results_files = []
    results_folder = os.path.join(base_output_folder, gene_name)

    # Find all "*.results_merged.tsv" files inside the gene folder (recursively)
    result_files = glob.glob(os.path.join(results_folder, "**", "results.tsv"), recursive=True)
    for result_file in result_files:
        list_results_files.append(result_file)

    merged_results_df = make_combine_results(gene_name, list_results_files)
    # Save that to a gz file
    merged_results_df.to_csv(os.path.join(base_output_folder, gene_name, "results_all_buffer.tsv"), sep="\t",
                             index=False)
    gzip_tsv_file(os.path.join(base_output_folder, gene_name, "results_all_buffer.tsv"),
                  os.path.join(base_output_folder, gene_name, "results_all.tsv.gz"))
    os.remove(os.path.join(base_output_folder, gene_name, "results_all_buffer.tsv"))

    transformed_results_df = transform_results_df(merged_results_df)
    diff_df = find_diff_in_results(transformed_results_df,
                                   os.path.join(base_output_folder, gene_name, "merged_results_filtered.tsv"))
